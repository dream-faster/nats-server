...
func BenchmarkJetStreamFastBatchPublish(b *testing.B) {
	...
	runFastBatchPublisher := func(b *testing.B, nc *nats.Conn, replyInbox string, messageSize int, subject string) (int, int) {
		...
		published, errors := 0, 0
		// attempt increments on every batch, successful or not, so a retried batch after
		// an error never reuses a batchID/sequence that the server may still consider
		// in-flight (which would skew results or hang waiting for a PubAck).
		attempt := 0
		b.ResetTimer()
		...
		for published < b.N {
			...
			attempt++
			batchID := fmt.Sprintf("bench-%d", attempt)
