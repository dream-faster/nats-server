...
//
// INVARIANT: when ce.poolBuf is set, each Entries[i].Data is a sub-slice of that
// pooled receive buffer. ReturnToPool releases the buffer back to aeBufPool, so
// the upper layer MUST NOT retain any reference to an Entry's Data (or a slice of
// it) past this call. All current consumers (meta/stream/consumer apply) satisfy
// this because they either copy the bytes into the message store or unmarshal them
// into freshly-allocated structures before returning the entry. Any new consumer
// that stashes Entry.Data without copying would read into a buffer that has been
// recycled for an unrelated network message.
